...
2. Validate: `cd marine-data-contract && npm run test:run && npm run build`
3. Update downstream packages importing `@omi/marine-data-contract`.
4. Validate each downstream package separately.
5. Do not validate everything at once unless breaking type rename.

## Expected output

- Path/type change summary + downstream impact list + per-package validation.

## Copilot note

- Use `grep_search` to find all usages of changed paths before editing downstream packages.
- Use `replace_string_in_file` with 3+ context lines for edits.
