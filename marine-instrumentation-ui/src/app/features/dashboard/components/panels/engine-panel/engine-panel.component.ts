...
import type { EnginePanelVm } from '../../../types/dashboard-vm';

@Component({
  selector: 'app-dashboard-engine-panel',
  standalone: true,
  ...
  styleUrls: ['./engine-panel.component.css'],
})
export class EnginePanelComponent {
  private readonly qualityService = inject(DataQualityService);

  @Input({ required: true }) vm!: EnginePanelVm;

  trackByMetric(_index: number, item: EnginePanelVm['metrics'][number]): string {
    return item.label;
  }

  get panelQuality(): DataQuality {
    if (!this.vm) return 'missing';
    if (this.vm.error) return 'stale';
    if (this.vm.isLoading) return 'warn';
    if (this.vm.metrics.length === 0) return 'missing';
  ...
  get fuelPercent(): number {
    if (this.vm.fuelLevel === null || this.vm.fuelLevel === undefined) return 0;
    return Math.max(0, Math.min(100, this.vm.fuelLevel * 100));
  }

  get fuelLevelClass(): string {
    const pct = this.fuelPercent;
    if (pct >= 50) return 'level-high';
    if (pct >= 20) return 'level-mid';
    return 'level-low';
  }
