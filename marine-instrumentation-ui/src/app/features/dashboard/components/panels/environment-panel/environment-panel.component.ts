...
import type { EnvironmentPanelVm } from '../../../types/dashboard-vm';

@Component({
  selector: 'app-dashboard-environment-panel',
  standalone: true,
...
export class EnvironmentPanelComponent {
  ...
  @Input({ required: true }) vm!: EnvironmentPanelVm;

  trackByMetric(_index: number, item: EnvironmentPanelVm['metrics'][number]): string {
    return item.label;
  }
