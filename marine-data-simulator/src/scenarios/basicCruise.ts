import type { Scenario, ScenarioPoint } from "../scenarios/scenario.js";
import { seedRandom } from "../engine/seedRandom.js";
import {
  PATHS,
  QualityFlag,
  type Position,
...
const jitter = (value: number, amplitude: number, random: () => number): number => {
  return value + (random() * 2 - 1) * amplitude;
};

const randomInRange = (min: number, max: number, random: () => number): number => {
...
const scheduleShallowEvent = (random: () => number): {
  durationSec: number;
  nextShallowSec: number;
  severity: number;
} => {
  return {
    durationSec: randomInRange(18, 28, random),
    nextShallowSec: randomInRange(140, 240, random),
    severity: randomInRange(4, 7, random),
  };
};

const scheduleGust = (random: () => number): {
  durationSec: number;
  nextGustSec: number;
  amplitude: number;
} => {
  return {
    durationSec: randomInRange(6, 12, random),
    nextGustSec: randomInRange(18, 40, random),
    amplitude: randomInRange(1.2, 3.2, random),
  };
};

const scheduleBatteryPhase = (random: () => number): {
  durationSec: number;
  mode: "charge" | "discharge";
  targetCurrent: number;
} => {
  const charge = random() < 0.4;
  return {
    durationSec: randomInRange(120, 220, random),
    mode: charge ? "charge" : "discharge",
    targetCurrent: charge ? randomInRange(-10, -4, random) : randomInRange(6, 18, random),
  };
};
...
const computeShallowDrop = (
  ...
  if (remainingSec <= 0 || durationSec <= 0) return 0;
...
export const createBasicCruiseScenario = (seed = 42): Scenario<BasicCruiseState> => {
  ...
  return {
    ...
    init: () => ({
      ...
      engineRpm: 35,
      engineRpmTarget: 35,
      coolantTemp: 273.15 + 82,
      oilPressure: 350_000,
      fuelLevel: 0.72,
      fuelRate: 0.000_002_78,
      waterTemp: 273.15 + 17.3,
      airTemp: 273.15 + 21.5,
      baroPressure: 101_325,
      humidity: 0.68,
      intruderLat: 42.2450,
      intruderLon: -8.7250,
      intruderSog: 6.5,
    ...
    tick: (state, dtSeconds, timestamp) => {
      const nextSogTarget = clamp(state.sogTarget + randomInRange(-0.08, 0.08, random), MIN_SOG, MAX_SOG);
      const nextSog = smoothValue(state.sog, nextSogTarget, 0.08);
      const nextCogTarget = wrapRadians(state.cogTarget + randomInRange(-0.01, 0.01, random));
      const nextCog = smoothAngle(state.cog, nextCogTarget, 0.12);
      const nextHeadingOffset = clamp(state.headingOffset + randomInRange(-0.005, 0.005, random), -0.2, 0.2);
      ...
      if (shallowRemainingSec <= 0 && nextShallowSec <= 0) {
        const scheduled = scheduleShallowEvent(random);
      ...
      const shallowDrop = computeShallowDrop(shallowRemainingSec, shallowDurationSec, shallowSeverity);
      const rawDepth = Math.max(MIN_DEPTH, jitter(profileDepth - shallowDrop, 0.15, random));
      const nextDepth = smoothValue(state.depth, rawDepth, 0.2);

      const nextTwsTarget = clamp(state.twsTarget + randomInRange(-0.1, 0.1, random), MIN_WIND_SPEED, MAX_WIND_SPEED);
      const nextTws = smoothValue(state.tws, nextTwsTarget, 0.12);
      const nextTwdTarget = wrapRadians(state.twdTarget + randomInRange(-0.012, 0.012, random));
      ...
      if (gustRemainingSec <= 0 && gustNextSec <= 0) {
        const scheduled = scheduleGust(random);
      ...
      const awaGeo = Math.atan2(-appU, -appV);
      const reportedAws = Math.max(0, jitter(awsInstant, 0.12, random));
      const reportedAwa = wrapRadians(angleDelta(nextHeading, awaGeo));
      const reportedTws = Math.max(0, jitter(twsInstant, 0.12, random));
      const reportedTwd = wrapRadians(jitter(nextTwd, 0.05, random));
      const reportedTwa = wrapRadians(angleDelta(nextHeading, reportedTwd));
      ...
      if (batteryPhaseRemainingSec <= 0) {
        const scheduled = scheduleBatteryPhase(random);
      ...
      const nextBatterySoc = clamp(state.batterySoc - deltaAh / BATTERY_CAPACITY_AH, 0.05, 1);
      const baseVoltage = 11.8 + nextBatterySoc * 1.4 + (batteryMode === "charge" ? 0.2 : 0);
      const sagVoltage = baseVoltage - nextBatteryCurrent * BATTERY_INTERNAL_RESISTANCE;
      const nextBatteryVoltage = clamp(jitter(sagVoltage, 0.05, random), BATTERY_VOLTAGE_MIN, BATTERY_VOLTAGE_MAX);
      ...
      if (shouldBroadcastStatic) intruderBroadcastTimer = 60;

      const rpmFactor = nextSog / MAX_SOG;
      const nextEngineRpmTarget = clamp(20 + rpmFactor * 25, 20, 45);
      const nextEngineRpm = smoothValue(state.engineRpm, nextEngineRpmTarget, 0.06);
      const coolantTarget = 273.15 + 80 + rpmFactor * 10;
      const nextCoolantTemp = smoothValue(state.coolantTemp, coolantTarget, 0.01) + (random() - 0.5) * 0.3;
      const oilTarget = 250_000 + rpmFactor * 200_000;
      const nextOilPressure = smoothValue(state.oilPressure, oilTarget, 0.04) + (random() - 0.5) * 5_000;
      const fuelConsumed = state.fuelRate * dtSeconds;
      const fuelTankSize = 0.200;
      const nextFuelLevel = clamp(state.fuelLevel - fuelConsumed / fuelTankSize, 0.01, 1);
      const nextFuelRate = state.fuelRate + (random() - 0.5) * 0.000_000_1;

      const nextWaterTemp = state.waterTemp + (random() - 0.5) * 0.02;
      const nextAirTemp = state.airTemp + (random() - 0.5) * 0.03;
      const nextBaroPressure = smoothValue(state.baroPressure, 101_325 + (random() - 0.5) * 200, 0.005);
      const nextHumidity = clamp(state.humidity + (random() - 0.5) * 0.003, 0.30, 0.95);
      ...
      const position: Position = { latitude: moved.latitude, longitude: moved.longitude };
      const reportedSog = Math.max(0, jitter(nextSog, 0.08, random));
      const reportedCog = wrapRadians(jitter(nextCog, 0.03, random));
      const reportedHeading = wrapRadians(jitter(nextHeading, 0.02, random));

      const points: ScenarioPoint[] = [
        makePoint(PATHS.environment.depth.belowTransducer, nextDepth, timestamp, QualityFlag.Good),
        makePoint(PATHS.environment.wind.speedApparent, reportedAws, timestamp, QualityFlag.Good),
        makePoint(PATHS.environment.wind.angleApparent, reportedAwa, timestamp, QualityFlag.Good),
        makePoint(PATHS.environment.wind.speedTrue, reportedTws, timestamp, QualityFlag.Good),
        makePoint(PATHS.environment.wind.angleTrueGround, reportedTwd, timestamp, QualityFlag.Good),
        makePoint(PATHS.environment.wind.angleTrueWater, reportedTwa, timestamp, QualityFlag.Good),
        makePoint(PATHS.electrical.batteries.house.voltage, nextBatteryVoltage, timestamp, QualityFlag.Good),
        makePoint(PATHS.electrical.batteries.house.current, nextBatteryCurrent, timestamp, QualityFlag.Good),
        ...
        makePoint(PATHS.navigation.courseOverGroundTrue, reportedCog, timestamp, QualityFlag.Good),
        makePoint(PATHS.navigation.headingTrue, reportedHeading, timestamp, QualityFlag.Good),
        makePoint(PATHS.propulsion.main.revolutions, jitter(nextEngineRpm, 0.2, random), timestamp, QualityFlag.Good),
